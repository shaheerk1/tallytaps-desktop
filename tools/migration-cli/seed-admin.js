...
async function run() {
  ...
  console.log('Seeding users, roles, permissions, and default workstation...');
  const seedResult = await seedAdminUser(database);
  console.log(JSON.stringify(seedResult, null, 2));

  console.log('Seeding default system settings...');
  const settingsResult = await seedDefaultSettings(database);
